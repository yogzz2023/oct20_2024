...
class KalmanFilterGUI(QWidget):
    ...
    def initUI(self):
        ...
        # PPI/RHI plot button
        self.ppi_rhi_button = QPushButton("PPI/RHI Plot")
        self.ppi_rhi_button.setFont(QFont('Arial', 10))
        self.ppi_rhi_button.setStyleSheet("background-color: #FF5722; color: white;")
        self.ppi_rhi_button.setFixedSize(100, 30)
        self.ppi_rhi_button.clicked.connect(self.show_ppi_rhi_plot)
        button_layout.addWidget(self.ppi_rhi_button)
    ...
    def show_ppi_rhi_plot(self):
        if not self.tracks:
            print("No tracks to plot.")
            return

        ax = self.canvas.figure.subplots()
        self.plot_ppi_rhi(self.tracks, ax)
        self.canvas.draw()

    def plot_ppi_rhi(self, tracks, ax):
        ax.clear()
        for track in tracks:
        ...
        ax.set_ylabel('Y/Z Coordinate')
        ax.set_title('PPI/RHI Plot')
        ax.legend()
