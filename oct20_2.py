...
class CAFilter:
    def __init__(self):
        self.Sf = np.zeros((9, 1))  # Filter state vector (x, y, z, vx, vy, vz, ax, ay, az)
    ...
    def initialize_filter_state(self, x, y, z, ax, ay, az, time):
        if not self.first_rep_flag:
            ...
            self.Meas_Time = time
            dt = self.Meas_Time - self.prev_Time
            self.ax = (self.Z1[0] - 2*self.Z2[0] + x) / (dt * dt)
            self.ay = (self.Z1[1] - 2*self.Z2[1] + y) / (dt * dt)
            self.az = (self.Z1[2] - 2*self.Z2[2] + z) / (dt * dt)
            self.Meas_Time = time
            self.second_rep_flag = True
        else:
            self.Z = np.array([[x], [y], [z]])
...
def main(input_file, track_mode, filter_option, association_type):
    ...
    # Select the appropriate filter based on the filter_option
    if filter_option == "CV":
